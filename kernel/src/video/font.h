/*
 * NeolyxOS Kernel - 8x16 Bitmap Font
 * 
 * Standard PC BIOS compatible font for text console
 * 
 * Copyright (c) 2025 KetiveeAI
 */

#ifndef NEOLYXOS_FONT_H
#define NEOLYXOS_FONT_H
...
/* Basic ASCII bitmap font (8x16 pixels per character)
 * Each character uses 16 bytes (one byte per row)
 * Bit 7 = leftmost pixel, Bit 0 = rightmost pixel
 */

static const uint8_t font_bitmap[95][16] = {
    /* Space (32) */
...
static inline const uint8_t* font_get_glyph(char c) {
    if (c < FONT_FIRST_CHAR || c > FONT_LAST_CHAR) {
        return font_bitmap[0]; /* Return space for invalid chars */
    }
    return font_bitmap[c - FONT_FIRST_CHAR];
}

#endif /* NEOLYXOS_FONT_H */
