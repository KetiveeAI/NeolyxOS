...
pub struct CppSupport {
    highlighter: CppSyntaxHighlighter,
}

impl CppSupport {
    pub fn new() -> Self {
        Self {
            highlighter: CppSyntaxHighlighter::new(),
        }
    }
}
...
impl LanguageSupport for CppSupport {
    ...
    fn lint(&self, source_file: &PathBuf) -> Result<Vec<LintMessage>> {
        ...
        // Use g++ with warnings enabled for linting
        ...
        if !output.status.success() {
            ...
            for line in error.lines() {
                if let Some(message) = self.parse_gpp_error(line, source_file) {
...
impl CppSupport {
    fn parse_gpp_error(&self, line: &str, file: &PathBuf) -> Option<LintMessage> {
        // Simple G++ error parsing
...
impl CppSyntaxHighlighter {
    pub fn new() -> Self {
        ...
        // C++ keywords (including C keywords)
        let cpp_keywords = vec![
            ...
            // C++ specific keywords
            "class", "namespace", "template", "typename", "virtual", "public", "private", "protected",
            "friend", "inline", "explicit", "mutable", "operator", "this", "new", "delete",
            "try", "catch", "throw", "using", "export", "typeid", "const_cast", "dynamic_cast",
            "reinterpret_cast", "static_cast", "bool", "wchar_t", "true", "false", "nullptr"
        ];
